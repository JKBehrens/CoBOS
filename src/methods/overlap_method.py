...
class OverlapSchedule(Schedule):
    ...
    def set_variables(self):
        ...
        for i, task in enumerate(self.job.task_sequence):
            ...
            self.task_assignment_var[task.id] = self.model.NewIntVar(
                0, number_agents - 1, f"task_{task.id}_assignment"
            )
            ...
            for phase in range(3):
                # set phase interval
                s = self.model.NewIntVar(
                    0, self.horizon, f"task_{task.id}_phase_{phase}_start"
                )
                e = self.model.NewIntVar(
                    0, self.horizon, f"task_{task.id}_phase_{phase}_end"
                )
                d = self.model.NewIntVar(
                    0, self.horizon, f"task_{task.id}_phase_{phase}_duration"
                )
                self.task_intervals[task.id].append(
                    self.model.NewIntervalVar(
                        s, d, e, f"phase_{phase}_task_{task.id}_interval"
                    )
                )
                ...
                for agent in self.all_agents:
                    phase_durations.append(
                        self.task_duration[agent][task.id][phase + 1]
                    )
                self.duration_constraints[task.id].append(
                    self.model.AddElement(
                        index=self.task_assignment_var[task.id],
            ...
            self.model.Add(
                self.task_intervals[task.id][0].EndExpr()
                == self.task_intervals[task.id][1].StartExpr()
            )
            self.model.Add(
                self.task_intervals[task.id][1].EndExpr()
                == self.task_intervals[task.id][2].StartExpr()
            )
            ...
            self.model.Add(
                duration
                == self.task_intervals[task.id][0].SizeExpr()
            ...
            for agent in self.all_agents:
                ...
                is_present = self.model.NewBoolVar(
                    f"is_present_task_{task.id}_agent_{agent}"
                )
                ...
                no_overlap_task_intervals.append(
                    self.model.NewOptionalIntervalVar(
                        self.task_intervals[task.id][0].StartExpr() + offset,
                        duration,
                        self.task_intervals[task.id][2].EndExpr() + offset,
            ...
            self.model.AddElement(
                index=self.task_assignment_var[task.id], variables=presence, target=True
            )
        ...
        for task, dependency_lst in self.dependencies.items():
            ...
            for dep in dependency_lst:
                ...
                self.dependencies_constraints[task][dep].append(
                    self.model.Add(
                        self.task_intervals[task][1].StartExpr()
                        >= self.task_intervals[dep][1].EndExpr()
                    )
                )
                self.dependencies_constraints[task][dep].append(
                    self.model.Add(
                        self.task_intervals[task][0].StartExpr()
                        >= self.task_intervals[dep][0].StartExpr()
                    )
                )
    ...
    def solve(self, current_time):
        ...
        if self.status == cp_model.OPTIMAL or self.status == cp_model.FEASIBLE:
            for task in self.job.task_sequence:
                if task.state in [TaskState.COMPLETED, TaskState.InProgress]:
                    if (
                        self.job.task_sequence[task.id].finish[0] < current_time
                        and task.state is TaskState.InProgress
                    ):
                        assert (
                            current_time - self.job.task_sequence[task.id].start
                        ) <= self.solver.Value(
                            self.task_intervals[task.id][2].EndExpr()
                        ) - self.solver.Value(
                            self.task_intervals[task.id][0].StartExpr()
                    ...
                    assert self.task_duration[agent][task.id][0] == self.solver.Value(
                        self.task_intervals[task.id][2].EndExpr()
                    ) - self.solver.Value(
                        self.task_intervals[task.id][0].StartExpr()
                    ), f"Duration of task {task.id} is wrong."
            ...
            for task in self.job.task_sequence:
                ...
                ends = [
                    self.solver.Value(self.task_intervals[dep][1].EndExpr())
            ...
            logging.error(
                f"No solution found. case {self.job.case} \nsolver_seed {self.seed}, dist_seed {self.job.seed}"
            )
            # self.constraint_debugger()
            raise ValueError(
                f"case {self.job.case}, solver_seed {self.seed}, dist_seed {self.job.seed}"
            )
        else:
            logging.error(self.model.Validate())
            logging.error(
                f"Something is wrong, status {self.solver.StatusName(self.status)} and the log of the solve"
            )
            raise ValueError(
                f"case {self.job.case}, solver_seed {self.seed}, dist_seed {self.job.seed}"
            )
        ...
        for task in self.job.task_sequence:
            task.agent = [
                self.agent_mapping[self.solver.Value(self.task_assignment_var[task.id])]
            ]

            if (
                (task.state == TaskState.UNAVAILABLE)
                or (task.state == TaskState.AVAILABLE)
                or (task.state is None)
            ):
                task.start = self.solver.Value(
                    self.task_intervals[task.id][0].StartExpr()
                )
                task.finish = [
                    self.solver.Value(self.task_intervals[task.id][2].EndExpr()),
                ...
                self.job.task_sequence[task.id].finish = [
                    ...
                    self.task_duration[task.agent[0]][task.id][2],
                    self.task_duration[task.agent[0]][task.id][3],
                ]
            else:
                task.start = self.solver.Value(
                    self.task_intervals[task.id][0].StartExpr()
                )
                task.finish[0] = self.solver.Value(
                    self.task_intervals[task.id][2].EndExpr()
                )
        ...
        for i, task in enumerate(self.job.task_sequence):
            if task.state in [TaskState.InProgress, TaskState.COMPLETED]:
                ...
                if task.state == TaskState.COMPLETED:
                    ...
                    completion_interval = [
                        [0, self.horizon],
                        [int(task.finish[0]), int(task.finish[0])],
                    ]
                    ...
                    self.model.Proto().variables[
                        self.task_intervals[task.id][2].EndExpr().Index()
                    ].domain[:] = []
                    self.model.Proto().variables[
                        self.task_intervals[task.id][2].EndExpr().Index()
                    ].domain.extend(
                ...
                self.model.Proto().variables[
                    self.task_intervals[task.id][0].StartExpr().Index()
                ].domain[:] = []
                self.model.Proto().variables[
                    self.task_intervals[task.id][0].StartExpr().Index()
                ].domain.extend(
                ...
                task.state == TaskState.AVAILABLE or task.state == TaskState.UNAVAILABLE
            ):
                intervals = [
                    [int(current_time), self.horizon],
                    [int(current_time), self.horizon],
                ]
    ...
    def refresh_task(self, model, task, preps, execution, completion):
        ...
        for phase in range(3):
            # set end of interval to current end time of task
            model.Proto().variables[
                self.task_intervals[task.id][phase].StartExpr().Index()
            ].domain[:] = []
            model.Proto().variables[
                self.task_intervals[task.id][phase].StartExpr().Index()
            ].domain.extend(
                cp_model.Domain(
                    phase_intervals[phase][0][0], phase_intervals[phase][0][1]
                ).FlattenedIntervals()
            )

            model.Proto().variables[
                self.task_intervals[task.id][phase].EndExpr().Index()
            ].domain[:] = []
            model.Proto().variables[
                self.task_intervals[task.id][phase].EndExpr().Index()
            ].domain.extend(
                cp_model.Domain(
                    phase_intervals[phase][1][0], phase_intervals[phase][1][1]
                ).FlattenedIntervals()
            )
    ...
    def change_agent_in_model(self, task):
        ...
        self.allowedAgents[task.id] = self.model.AddAllowedAssignments(
            ...
            tuples_list=[
                tuple([v])
                for v in self.task_assignment[task.id]
                if v != self.agent_mapping[task.agent[0]]
            ],
        )
    ...
    def set_solver(self):
        ...
        solver.parameters.log_search_progress = (
            True if logging.getLogger().level == 10 else False
        )
        solver.parameters.search_branching = cp_model.AUTOMATIC_SEARCH
        return solver

    def set_list_of_possible_changes(
        self, available_tasks, agent_name, current_time, **kwargs
    ):
        makespans = []
    ...
    def decide(self, observation_data, current_time):
        ...
        if (
            self.is_there_IDLE_agent(observation_data)
            and self.are_there_available_tasks()
        ):
            ...
            if status == cp_model.OPTIMAL or status == cp_model.FEASIBLE:
                ...
                for index, [
                    ...
                    logging.debug(
                        f"TIME: {current_time}. Is {agent_name} available? {agent_state}"
                    )
                    if agent_state == AgentState.IDLE:
                        decision[agent_name] = self.find_task(
                            agent_name, agent_rejection_tasks, current_time
                        )
                    elif agent_state == AgentState.REJECTION:
                        coworker = observation_data[index - 1]
                        self.change_agent(
                            task=agent_current_task,
                            coworker_name=coworker[0],
                            current_time=current_time,
                        )
                        decision[agent_name] = self.find_task(
                            agent_name, agent_rejection_tasks, current_time
                ...
                if (
                    max(task.finish[0] for task in self.job.task_sequence)
                    > self.horizon
                ):
                    logging.error(
                        f"Horizon is incorrect. Horizon {self.horizon}, max finish {max(task.finish[0] for task in self.job.task_sequence)}"
                    )
                logging.error(
                    f"Something is wrong, status {solver.StatusName(status)} and the log of the solve"
                )
