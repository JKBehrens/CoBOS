...
class Schedule(Solver):
    ...
    def __init__(self, job: Job, seed: int):
        ...
        self.border_constraints = [
            [[0, 0, 0, 0, 0]] * self.job.task_number
        ] * self.job.task_number
        ...
        for i, task in enumerate(self.job.task_sequence):
            ...
            self.duration[i] = self.model.NewIntVarFromDomain(
                cp_model.Domain.FromIntervals(
                    [
                        [self.task_duration["Human"][task.id][0]],
                        [self.task_duration["Robot"][task.id][0]],
                    ]
                ),
                "duration" + suffix,
            )
            ...
            if task.agent == "Human":
                ...
                self.soft_constr[i] = self.model.NewIntVar(
                    0, prob, "rejection" + suffix
                )
                self.model.Add(self.soft_constr[i] == prob).OnlyEnforceIf(
                    self.human_task_bool[i]
                )
                self.model.Add(self.soft_constr[i] == 0).OnlyEnforceIf(
                    self.human_task_bool[i].Not()
                )
            ...
            self.all_tasks[task.id] = task_info(
                start=self.start_var[i],
                end=self.end_var[i],
    ...
    def set_constraints(self):
        ...
        for i, task in enumerate(self.job.task_sequence):
            self.duration_constraints[i][0] = self.model.Add(
                self.duration[i] == self.task_duration["Human"][task.id][0]
            ).OnlyEnforceIf(self.human_task_bool[i])
            self.duration_constraints[i][1] = self.model.Add(
                self.duration[i] == self.task_duration["Robot"][task.id][0]
            ...
            for j in range(self.job.task_number):
                if self.job.task_sequence[j].id != task.id:
                    same_agent = self.model.NewBoolVar(
                        f"same_agent_4_tasks_{task.id}_{j}"
                    )
                    self.model.Add(
                        self.human_task_bool[i] == self.human_task_bool[j]
                    ).OnlyEnforceIf(same_agent)
                    self.model.Add(
                        self.human_task_bool[i] != self.human_task_bool[j]
                    ).OnlyEnforceIf(same_agent.Not())
                    ...
                    self.border_constraints[i][j][0] = self.model.Add(
                        self.all_tasks[j].start >= self.all_tasks[i].end
                    ).OnlyEnforceIf([condition.Not(), same_agent, after])
                    self.border_constraints[i][j][1] = self.model.Add(
                        self.all_tasks[j].end <= self.all_tasks[i].start
                    ...
                    k1 = (
                        self.task_duration["Human"][task.id][1]
                        + self.task_duration["Human"][task.id][2]
                        - self.task_duration["Robot"][dependent_task_id][1]
                    )
                    k2 = (
                        self.task_duration["Robot"][task.id][1]
                        + self.task_duration["Robot"][task.id][2]
                    ...
                    self.model.Add(k == k1).OnlyEnforceIf(
                        [self.human_task_bool[i], condition]
                    )
                    self.model.Add(k == k2).OnlyEnforceIf(
                        [self.human_task_bool[i].Not(), condition]
                    )
                    logging.debug(f"task i {i}, dt j {j}, k = {k2}")
                    self.border_constraints[i][j][3] = self.model.Add(
                        self.all_tasks[dependent_task_id].end
                        >= self.all_tasks[task.id].end + k
                    ...
                    self.border_constraints[i][j][4] = self.model.Add(
                        self.all_tasks[dependent_task_id].start
                        >= self.all_tasks[task.id].start
                    ).OnlyEnforceIf([condition, same_agent.Not()])

        # Makespan objective.
        obj_var = self.model.NewIntVar(0, self.horizon, "makespan")
        self.model.AddMaxEquality(
            obj_var, [self.all_tasks[i].end for i, task in enumerate(self.all_tasks)]
        )
    ...
    def refresh_variables(self, current_time):
        ...
        """
        for i, task in enumerate(self.job.task_sequence):
            if (task.id not in self.tasks_with_final_var) and (
                task.state in [TaskState.InProgress, TaskState.COMPLETED]
            ):
                ...
                if (
                    self.duration_constraints[i][0].Proto()
                    in self.model.Proto().constraints
                ):
                    # logging.debug(f'Duration constraints has been deleted, Task i {i}')
                    for j in range(2):
                        self.model.Proto().constraints.remove(
                            self.duration_constraints[i][j].Proto()
                        )
                # Cancel constraints
    ...
    def solve(self, current_time):
        ...
        assigned_task_info = collections.namedtuple(
            "assigned_task_info", "start end task_id agent"
        )
        ...
        if self.status == cp_model.OPTIMAL or self.status == cp_model.FEASIBLE:
            ...
            for i, task in enumerate(self.job.task_sequence):
                ...
                self.assigned_jobs[agent].append(
                    assigned_task_info(
                        start=self.solver.Value(self.all_tasks[i].start),
                        end=[
                            self.solver.Value(self.all_tasks[i].end),
                            self.task_duration[agent][task.id][1],
                            self.task_duration[agent][task.id][2],
                            self.task_duration[agent][task.id][3],
                        ],
                        task_id=i,
                        agent=agent,
                    )
            ...
            self.rescheduling_run_time.append(
                [
                    current_time,
    ...
    def find_task(self, agent_name, agent_rejection_tasks, current_time):
            ...
            if worker != agent_name:
                for task in self.schedule[worker]:
                    if (
                        ...
                        and task.universal
                        and task.id not in agent_rejection_tasks
                    ):
        ...
        if len(possible_tasks) != 0:
            ...
            makespan_and_task = self.set_list_of_possible_changes(
                possible_tasks, agent_name, current_time
            )
            if makespan_and_task and makespan_and_task[0][0] < self.current_makespan:
                self.change_agent(
                    task=makespan_and_task[0][1],
                    coworker_name=agent_name,
                    current_time=current_time,
                )
