...
class ControlLogic:
    ...
    def get_observation_data(self):
        ...
        for agent in self.agents:
            if (
                agent.state == AgentState.REJECTION
                or agent.state == AgentState.ACCEPTANCE
            ):
                pass
            ...
            try:
                output.append(
                    [
                        ...
                        agent.state,
                        agent.current_task,
                        np.array(agent.rejection_tasks)[:, 0],
                    ]
                )
            except IndexError:
    ...
    def run(
        ...
        while True:
            ...
            try:
                selected_task: dict[str, Task | None] = self.solving_method.decide(
                    observation_data, self.current_time
                )
            except ValueError as e:
                msg = f"{e} \nsim_seed {self.agents[0].seed}"
                logging.error(msg=msg)
                break
            self.decision_making_duration.append(
                time.perf_counter() - decision_making_start
            )
            ...
            for task in selected_task.values():
                ...
                if task.state == TaskState.ASSIGNED:
                    raise ValueError(
                        f"task {task.id} was left in state {task.state}. It will not be possible to execute the task."
                    )
            ...
            if self.current_time == 0:
                try:
                    ...
                    self.output_data = []
                    self.output_data.append(
                        copy.deepcopy(self.schedule_as_dict(hierarchy=True))
                    )
                except KeyError:
                    self.output_data.append([])
                except ValueError:
            ...
            if offline_video:
                # save current state
                if self.current_time == 0:
                    self.init_schedule = copy.deepcopy(
                        self.schedule_as_dict(hierarchy=True)
                    )

                if (
                    self.plot.current_time + 2 == self.current_time
                    ...
                    self.plot.plot_schedule(
                        self.plot.save_path.joinpath(f"{self.current_time:03d}.png"),
                        video=True,
        ...
        if offline_video:
            ...
            self.plot.save_data()
            os.system(
                "ffmpeg -framerate 5 -pattern_type glob -i '*.png' -c:v libx264 -pix_fmt yuv420p simulation.mp4"
            )
        ...
        if save2file:
            ...
            logging.info(
                f"The data was saved to {initial_and_final_schedule_save_file_name}"
            )
