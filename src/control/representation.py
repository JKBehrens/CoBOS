...
class Task(BaseModel):
    ...
    def get_phase_distribution(self, phase, distribution_param, rand_gen: np.random.Generator|None=None,  seed=None):
        if rand_gen is None:
            rand_gen = np.random.default_rng(seed + self.id)
    ...
    def get_phase_duration(self, phase: int, distribution_param, rand_gen: np.random.Generator|None= None,  seed:int|None=None):
        if rand_gen is None:
            assert isinstance(seed, int), "Either rand_gen or seed must be specified. None is given."
            rand_gen = np.random.default_rng(seed + self.id)
        assert isinstance(rand_gen, np.random.Generator)
