...
class Web_vis:
    ...
    def update_gantt_chart(self):
        ...
        tick = (
            ...
            .encode(x="End:Q", y="Agent:N")
        )

        text = (
            alt.Chart(pandas_data)
            .mark_text(dx=7, dy=0, color="white")
            .encode(
                x="Start",
                y="Agent",
                # detail='site:N',
        ...
        try:
            self.chart_placeholder.altair_chart(
                bar_chart + current_time_rule + tick + text, use_container_width=True
            )
        except RuntimeError:
            pass
    ...
    def update_info(self, agent, start=False):
        if start:
            string = (
                f"{agent.name}: place {agent.current_task.action['Object']} to {agent.current_task.action['Place']}.   "
