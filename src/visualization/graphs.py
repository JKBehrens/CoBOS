...
class Vis:
    def __init__(self, horizon=None, data=None, from_file=False):
        ...
        # positions
        self.y_pos_and_text = {
            True: {"Human": [7.5, 8.8, 6.6], "Robot": [4.5, 5.8, 3.6]},
            False: {"Human": [10.5, 11.8, 9.6], "Robot": [1.5, 2.8, 0.6]},
        }
        self.color = {
            None: "lightcoral",
            -1: "lightcoral",
    ...
    def set_plot_param(self, title, gs, lim=None, reschedule_num=False):
        ...
        colors1 = [
            "royalblue"
        ]  # 'lightsteelblue', 'cornflowerblue',['thistle', 'plum', 'violet']
    ...
    def plot_schedule(
        self, file_name: Path | str = "", video=False, stat=None, case=None
    ):
        ...
        if "simulation" in file_name.stem:
            ...
            title = [
                "Gantt Chart: initial",
                "Gantt Chart: final (same sampling seed)",
        ...
        for i, position in enumerate(positions[0]):
            if "comparison" in file_name.stem or (video and stat is not None):
                if i != 0:
                    self.set_plot_param(
                        title[i], gs[i, :], lim=horizon, reschedule_num=True
                    )
                    colors = {0: "red", 1: "blue"}
                    position = {0: 14, 1: 15.5}
                    try:
                        if stat is None:
                            for idx, solver_info in enumerate(
                                self.data["statistics"][i - 1]["solver"]
                            ):
            ...
            for agent in local_data[i]:
                for task in local_data[i][agent]:
                    position_y, task_name_y, action_y = self.y_pos_and_text[
                        task["universal"]
                    ][agent]
                    ...
                    if (
                        video
                        and task["state"] == 1
                        and task["finish"][0] < self.current_time
                        and i != 0
                    ):
                        task["finish"][0] = self.current_time
                        self.gnt.broken_barh(
                            [(task["start"], task["finish"][0] - task["start"])],
                            [position_y - 1.2, 2.4],
                            facecolors=color[0],
                        )
                    else:
                        ...
                        self.gnt.broken_barh(
                            [(task["start"], preps_duration)],
                            [position_y - 1.2, 2.4],
                            facecolors=color[0],
                        )
                        self.gnt.broken_barh(
                            [(task["start"] + preps_duration, task["finish"][2])],
                            [position_y - 1.2, 2.4],
                            facecolors=color[1],
                        )
                        self.gnt.broken_barh(
                        ...
                        self.gnt.annotate(
                            "",
                            xy=((task["finish"][0]), position_y - 1.3),
            ...
            if video and i != 0:
                self.gnt.annotate(
                    "",
                    xy=(self.current_time, 0),
                    xytext=(self.current_time, 13),
    ...
    def online_plotting(self):
        ...
        bar_chart = (
            ...
            .properties(title="Gantt Chart", width=max(self.data["End"]))
        )
        current_time_rule = (
            alt.Chart(pd.DataFrame({"current_time": [self.current_time]}))
            .mark_rule(color="red")
            .encode(x="current_time", size=alt.value(2))
        )

        self.chart_placeholder.altair_chart(
            bar_chart + current_time_rule, use_container_width=True
        )
            ...
            for task in local_data[agent]:
                if task["conditions"]:
                    for j in task["conditions"]:
                        G.add_edges_from(
                            [(get_task_from_id(j, local_data), task["action"]["place"])]
                        )
        ...
        for color in colors.keys():
            for state in states.keys():
                nx.draw_networkx_nodes(
                    G,
                    pos,
                    nodelist=list(set(colors[color]) & set(states[state])),
                    node_color=color,
                    edgecolors=state_color_map[state],
                    node_size=node_size,
