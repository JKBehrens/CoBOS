...
if __name__ == "__main__":
    ...
    if args.log_error:
        ...
        lvl = logging.INFO
    logging.basicConfig(level=lvl, format=f"%(levelname)-8s: - %(message)s")
    ...
    if args.comparison:
        if args.change_job:
            control_logic = ControlLogic(
                job=copy.deepcopy(job), agents=agents, method=solving_method
            )
            schedule1, stat1 = control_logic.run(
                experiments=True,
                save2file=save2file,
                animation=save_gt,
                offline_video=video,
            ...
            schedule2, stat2 = control_logic.run(
                experiments=True,
                save2file=save2file,
                animation=save_gt,
                offline_video=video,
        ...
        try:
            with open(file_name, "w") as outfile:
                json.dump(
                    {
                        "schedule": schedule1 + [schedule2[1]],
            ...
            with open(file_name, "w") as outfile:
                json.dump(
                    {"schedule": schedule1 + schedule2, "statistics": [stat1, stat2]},
                    outfile,
                    indent=4,
                )
                logging.info(f"Save data to {file_name}")
        ...
        control_logic = ControlLogic(job=job, agents=agents, method=solving_method)
        schedule, statistics = control_logic.run(
            experiments=True,
            save2file=save2file,
            animation=save_gt,
            offline_video=video,
